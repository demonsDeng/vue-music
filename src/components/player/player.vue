  ...
  <div class="player" v-show="playList.length>0">
    ...
    >
      <div class="normal-player" v-show="fullScreen">
        ...
        <div class="middle" @touchstart.prevent="lyricTouchStart"
        ...
        >
          ...
          <scroll ref="lyricWrapper" :data="currentLyric && currentLyric.lines" class="middle-r">
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p :class="{'current':currentLyricNum === index}" ref="lyricLine" class="text" v-for="(line,index) in currentLyric.lines">{{line.txt}}</p>
              </div>
            </div>
          </scroll>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      lyricTouchMove(e) {
        ...
        const deletaX = touch.pageX - this.startX
        const deletaY = touch.pageY - this.startY
        if (Math.abs(deletaX) < Math.abs(deletaY)) {
          return
        }
        const left = this.currentShow === 'cd' ? 0 : -window.innerWidth
        const width = Math.min(0, Math.max(-window.innerWidth, left + deletaX))
      ...
      lyricTouchEnd() {
        console.log(this.touch.percent, this.currentShow)
        let offsetWidth
        let opacity
      ...
      handlerLyric({lineNum, txt}) {
        ...
        if (lineNum > 5) {
          let lineElm = this.$refs.lyricLine[lineNum - 5]
          this.$refs.lyricWrapper.scrollToElement(lineElm, 1000)
        } else {
